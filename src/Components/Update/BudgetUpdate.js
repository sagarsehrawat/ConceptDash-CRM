...
function BudgetUpdate() {
    ...
    useEffect(() => {
        setIsLoading(true);
          const call = async () => {
            await axios.get('https://conceptdashcrm-env.eba-bjgvjq2h.ca-central-1.elasticbeanstalk.com/api/get/budgets', {headers:{'auth':'Rose '+ localStorage.getItem('auth')}}).then((res) => {
