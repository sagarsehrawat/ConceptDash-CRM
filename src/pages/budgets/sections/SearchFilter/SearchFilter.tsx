...
const SearchFilter = ({
    ...
    const [prevFilter, setprevFilter] = useState<FilterType>({ dept: [], cat: [], budgetCategory: [] });
    ...
    const filterSize = () => {
        return filter.budgetCategory.length + filter.cat.length + filter.dept.length;
    }
    ...
    return (
        <>
            <div className='d-flex flex-row justify-content-between' style={{ marginTop: "8px", marginBottom: "24px", marginLeft: "32px", marginRight: "32px" }}>
                <div className='d-flex flex-row'>
                    ...
                    <Button
                        ...
                        style={{ backgroundColor: filterSize() > 0 ? "#DBDBF4" : "white", marginRight: "12px" }}
                        ...
                        <div style={{ width: "786px", height: "356px", boxShadow: "0px 4px 25px rgba(0, 0, 0, 0.08)", borderRadius: "6px" }}>
                            ...
                            <div
                                ...
                                <div className='filter-container filter-subcontainer'>
                                    <p className='filter-subheading'>Budget Category {filter.budgetCategory.length === 0 ? "" : `/${filter.budgetCategory.length}`}</p>

                                    {isLoading2[0] ? <LoadingSpinner /> : 
                                    <>
                                    <div style={{ backgroundColor: filter.budgetCategory.includes("Design") ? "#DBDBF4" : "#F7F7F9" }} className='filter-sub-subcontainer' onClick={() => handleFilter('dept', "Design")}><p className='filter-body-text'>Design</p></div>
                                    <div style={{ backgroundColor: filter.budgetCategory.includes("Construction") ? "#DBDBF4" : "#F7F7F9" }} className='filter-sub-subcontainer' onClick={() => handleFilter('dept', "Construction")}><p className='filter-body-text'>Construction</p></div>
                                    </>
                                    }
                                </div>
            ...
            {showAdd && <AddBudget show={showAdd} onHide={() => setShowAdd(false)} setApi={setApi} cityId={cityId} />}
        </>
    )
}
