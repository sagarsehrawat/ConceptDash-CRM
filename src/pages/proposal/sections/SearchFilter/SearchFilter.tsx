...
const SearchFilter = ({
  ...
  const [cities, setcities] = useState<
    Array<{ City_ID: number | string; City: string }>
  >([]);
  ...
  const [employees, setemployees] = useState<
    Array<{ Employee_ID: string | number; Full_Name: string }>
  >([]);
  ...
  const handleFilter = (key: keyof FilterType, value: string | number) => {
    setFilter((prevFilter: FilterType) => {
      ...
      if (key !== "bookmark") {
        ...
        updatedFilter[key] = !updatedFilter[key];
      }
  ...
  const filterSize = () => {
    return (
      ...
      (filter.bookmark === true ? 1 : 0)
    );
  };

  const bookmarkFilterButton = (
    <div
      style={{ backgroundColor: filter.bookmark ? "#DBDBF4" : "#F7F7F9" }}
      className="filter-sub-subcontainer"
      onClick={() => {
        handleFilter("bookmark", 0);
      }}
    >
      <p className="filter-body-text">Bookmarks</p>
    </div>
  );
            ...
            >
              <div className="filter-container filter-subcontainer">
                ...
                {isLoading2[0] ? (
                  ...
                  cities.map((e) => {
                    if (
                      e.City.toLowerCase().startsWith(searchCity.toLowerCase())
                    ) {
                      return (
                        <div
                          style={{
                        ...
                        >
                          <p className="filter-body-text">{e.City}</p>
                        </div>
                      );
                    } else {
                      return <></>;
                ...
                {isLoading2[1] ? (
                  ...
                  depts.map((e) => {
                    return (
                      <div
                        style={{
                          backgroundColor: filter.dept.includes(e.Department_ID)
                            ? "#DBDBF4"
                            : "#F7F7F9",
                        }}
                        className="filter-sub-subcontainer"
                ...
                {isLoading2[2] ? (
                  ...
                  projectCategories.map((e) => {
                    return (
                      <div
                        style={{
                          backgroundColor: filter.cat.includes(e.Project_Cat_ID)
                            ? "#DBDBF4"
                            : "#F7F7F9",
                        }}
                        className="filter-sub-subcontainer"
                        onClick={() => handleFilter("dept", e.Project_Cat_ID)}
                      >
                        <p className="filter-body-text">{e.Project_Category}</p>
                      </div>
                    );
                  })
                )}
                ...
                {isLoading2[2] ? (
                  ...
                  employees.map((e) => {
                    return (
                      ...
                      >
                        <p className="filter-body-text">{e.Full_Name}</p>
                      </div>
                    );
                  })
                )}
              ...
              <div className="filter-container filter-subcontainer">
                <p className="filter-subheading">Bookmarks </p>
                {isLoading2[2] ? <LoadingSpinner /> : bookmarkFilterButton}
              </div>
              ;
            </div>
