...
export default function List({
  ...
  function handleUnselectedClick(list, index) {
    ...
    b.sort(catSort);
    a.sort(catSort);
    setUnselectedLists((prev) => a);
    setUnselectedListsCopy((prev) => a);
    setSelectedLists((prev) => b);
    setSelectedListsCopy((prev) => b);
  }
  ...
  function handleSelectedClick(list, index) {
    ...
    b.sort(catSort);
    a.sort(catSort);
    setSelectedLists((prev) => a);
    setSelectedListsCopy((prev) => a);
    setUnselectedLists((prev) => b);
    setUnselectedListsCopy((prev) => b);
  }
      ...
      {selectedListsCopy.map((list, index) => (
        <div style={styles.listContainer()} key={index.toString()}>
          ...
          <div style={{ display: "flex", alignItems: "center" }}>
            <img
              src={`https://ui-avatars.com/api/?name=${list?.CATEGORY}&color=fff&background=979DA9&rounded=true`}
              alt="img"
              width={36}
              height={36}
            />
